...
def run():
    ...
    onecode.Logger.info(f"Detection des anomalies (facteur MAD = {facteur_mad})...")
    for elem in indicateurs:
    ...
    for i, (elem, cmap) in enumerate(zip(elements_carte, cmaps)):
        ...
        ax.set_xlabel('X (m)')
        ax.set_ylabel('Y (m)')
    fig.suptitle(f'Anomalies geochimiques - seuil Mediane+{facteur_mad}xMAD')
    plt.tight_layout()
    ...
    onecode.Logger.info(f"Top {(1-quantile_top)*100:.0f}% : {len(top)} points (seuil={seuil_top:.3f})")
    ...
    ax.set_title(
        f'Potentiel mineral : Au({p_au*100:.0f}%) + As({p_as*100:.0f}%) '
